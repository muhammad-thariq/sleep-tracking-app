...
Second, wake you up properly. We added a **Smart Alarm** that goes off inside a wake window you choose, plus a **Wake-Up Challenge**: a small arithmetic puzzle you have to get right before the alarm shuts up. We kept running into the problem of swiping our own alarms off in our sleep, so this was our fix for it.

One thing we cared about from the start: everything stays on the phone. There's no account and no server. Even the microphone is only ever read as a loudness number, and we throw the actual audio away and never save or send it anywhere.
...
### 2. Track (`/track`)
The live screen while a manual session is running. There's a **RECORDING / PAUSED** pill, a running timer, and two sensor cards you can flip on independently: **Audio** (the mic, for picking up noise) and **Motion** (the accelerometer, for catching restless movement). **Pause** and **Stop** are there too, and stopping saves the session.
...
<table>
  <tr>
    <td><img src="docs/screenshots/03-analysis-thu-jun11.jpeg" width="240" alt="Analysis for Thu Jun 11: 5h 19m Optimal, auto-detected." /></td>
...
Where you set up your alarms. Each card shows the wake time, the wake window (for example `Window: 05:50 to 06:30`), an on/off switch, an edit button, and a row of weekday toggles. We support two kinds: **Smart Wake**, which fires at the start of the window, and **Standard**, which fires at the exact time. The button at the bottom adds a new one.

<img src="docs/screenshots/06-smart-alarms.jpeg" width="300" alt="Smart Alarms screen with two Smart Wake alarms (06:30 with a wake window, and 16:51) and an Add New Alarm button." />

### 5. Alarm Ringing, full screen (`/alarm-ringing`)
...
Hit **Solve to Stop** and this sheet slides up. The math equation on screen is **the puzzle**: a random arithmetic problem (like `3 × 10 = ?`) with four choices. The alarm only stops once you tap the right answer, and tapping a wrong one gives you a fresh problem instead. The point is that you can't just mash a button to make it stop. You have to wake up enough to actually do the sum.

<img src="docs/screenshots/08-wake-up-challenge.jpeg" width="300" alt="Wake-Up Challenge sheet asking 3 times 10 with four choices: 25, 30, 33, 35." />
...
- **Sleep Quality Score:** a single number from 0 to 100, built from how much you were awake, how many disturbances there were, and how far off 8 hours you slept.
...
- **Offline and private:** everything is stored locally, the audio is loudness-only, and nothing leaves the phone.
...
### Sleep Quality Score
The code lives in [`lib/services/scoring.dart`](lib/services/scoring.dart). We start at 100 and subtract penalties, then keep the result between 0 and 100:
...
For example, the 20h 55m night in the screenshots gets hit hard for being roughly 13 hours off the 8-hour target, which is why it lands at **38 (Needs Rest)**.

### Auto-Detection
This is handled by [`idle_window_detector.dart`](lib/services/idle_window_detector.dart) and [`auto_tracking_service.dart`](lib/services/auto_tracking_service.dart). We run it once when the app launches and then hourly while it's open (no background daemon, since we wanted to keep it simple):
...
3. Save that as an auto session. We never overwrite a manual one (those are more trustworthy), and we only replace an existing auto session if the new window is clearly longer.

### Smart Alarm Scheduling
This lives in [`alarm_scheduling.dart`](lib/services/alarm_scheduling.dart) and [`alarm_service.dart`](lib/services/alarm_service.dart).

- Smart alarms fire at the **start** of the wake window (`time − windowMinutes`), while standard alarms fire right on `time`.
...
From [`manual_tracking_service.dart`](lib/services/manual_tracking_service.dart) and [`disturbance_debouncer.dart`](lib/services/disturbance_debouncer.dart):

- **Noise:** loudness above `−20 dBFS` held for at least 2 seconds counts as a noise event.
- **Motion:** when the accelerometer's shakiness over a roughly 10-second window crosses a threshold, that's a movement event.
...
### Wake-Up Challenge
Built in [`alarm_ringing_screen.dart`](lib/screens/alarm_ringing_screen.dart). We generate a random `+`, `−`, or `×` problem with small, friendly numbers and four unique choices. The sheet can't be swiped away, a wrong answer buzzes and swaps in a new problem, and only the correct answer closes it and stops the alarm.
...
You'll need the [Flutter SDK](https://docs.flutter.dev/get-started/install) (Dart `^3.12.0`) and an Android device or emulator. Android is our main target, since the alarm, usage-access, and foreground-service features are Android-specific.
...
On the first launch, you'll be asked to grant Usage Access (for auto-detection) and the mic and activity permissions (for manual tracking).
...
- We made the wake-up puzzle **multiple-choice** rather than asking you to type the answer, because picking a number is much easier than recalling one when you're groggy.
